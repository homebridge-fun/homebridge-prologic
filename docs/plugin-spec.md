...
| ↳ Cockpit dead code | Done | The unused `num()` helper and the dead `.pill.ctl` CSS rule no longer exist — cleaned up as part of later cockpit passes. |
| ↳ Optional hardening | Low (checked 2026-08) | `aqualogic==3.4` is pinned in `deploy/install-pad.sh`'s pip install — done. `_install_key_burst` (the fragile source-string monkeypatch this note warned about) was deleted entirely in the 0.7.2 legacy-`rs485` removal — moot. Heater enable is already body-aware (`_ac_heater_enable(which, on)` routes pool vs spa, with a menu-nav fallback for the non-active body). `chlorinator_percent`/`spa_chlorinator_percent` are both consistently `Optional[float]` — nothing to normalize. Only genuinely open: hoist magic numbers (600ms debounce, 35% floor, timers) to named constants — cosmetic, still low priority. |
| **Heater setpoint dial: no instant revert on a failed write** | Open — deliberately deferred (2026-08) | Every other write (circuits, heater enable, Super Chlorinate, chlorinator %, VSP slots, spa speed) now confirms the real result and reverts the optimistic UI immediately on failure (§5.5–§5.8). The heater **setpoint** dial (`/heater/<which>/setpoint`) is the one exception: it's debounced (HomeKit fires a write per drag tick; coalescing into one ~15s menu-nav write is required to avoid flooding the panel) and returns `202 Accepted` **before the physical write even starts** — so there is no request/response cycle left to carry a failure back to the caller. **Current behavior:** the sidecar still tracks the truth correctly — `set_heater()` records whatever `_step_to` actually confirmed (not the blind target) into `state.pool/spa_setpoint_f`, and logs a warning if it didn't reach the requested value. So a dropped-press undershoot is never permanently wrong, it just corrects on the **next status poll** (~poll interval after the ~15s write completes) instead of reverting instantly like a toggle does. **Why we're leaving it:** fixing the instant-revert case would mean restructuring the debounce architecture (e.g. some way to push the outcome back to the specific client that requested it, after the fact) — real scope for a dial where "converges within a few seconds of the drag settling" is an acceptable UX, unlike a toggle that visibly snaps to the wrong state. Revisit if it's ever actually observed to feel wrong in practice. |
...
| RS-485 smart bridge (writes) | **DONE — pad daemon: 100% keypress landing 2026-07-10** | Full architecture in §4.4. Direct serial from a pad Pi Zero 2 W solved the write problem the TCP bridge couldn't. Root cause of the residual ~33% landing was the **FTDI USB-serial `latency_timer` (16ms default)**, not the protocol or predelay — pinned to **1ms** via `deploy/99-ftdi-low-latency.rules` → **30/30 & 20/20 presses landed (100%)** vs the TCP bridge's ~60% single-shot. `aqualogic==3.4`'s `_write_to_serial()` `.send()`→`.write()` bug is patched in the daemon; `LONG_DISPLAY_UPDATE` capture added so menu-nav frames are visible. Daemon (`sidecar/rs485_bridge.py`, systemd `pool-bridge`) is autonomous + reboot-hardened; installer + runbook in `deploy/README-PAD.md`. Diagnostics kept: `rs485_bench.py`, `rs485_sniff.py`, `serial_smoketest.py`. Both former **Open** items resolved: `aqualogic==3.4` is pinned in `deploy/install-pad.sh`. HEATER_1 (a wireless-frame key, >0xffff) sends fine over the daemon's `aqualogic.send_key()` fallback (`rs485_bridge.py`'s `send_key()`); the reliability issue we chased turned out to be the SIDECAR reading its confirmation too early (a single instant read raced the ~200ms LED broadcast) — fixed 2026-08 by polling the armed-state bit until it settles before confirming (§5.5), not a frame-type problem. |
| rs485bridge backend | **DONE — production primary** | `RS485BridgeBackend` in `pool_service.py` drives the pad daemon over HTTP/Tailscale, mirroring `AquaConnectBackend` (`lcd` + `send_nav_key`) so `MenuNavigator` / `_ac_set_circuit` / prefetch work unchanged. `--backend rs485bridge --rs485bridge-host <pad-host>` — normally the pad's MagicDNS name, which survives a tailnet-IP change; a raw `<pad-tailnet-ip>` also works. Set in the Homebridge UI as `rs485bridgeHost`; never hardcoded; token via `RS485_BRIDGE_TOKEN` (never persisted). §4.4. Plugin `config.schema.json` + `platform.ts` wiring, HEATER_1 path, and production cutover all shipped; it's the live primary backend. |
