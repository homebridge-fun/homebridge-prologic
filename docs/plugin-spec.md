...
| Conditional startup sweep | Backlog | Now that bus state persists to `state_cache.json` and restores on boot, the full `read_all_settings()` sweep on every restart is redundant when nothing changed. Future: skip the sweep (trust the cache) or do a cheap scroll-based diff first and only menu-read fields that disagree with the persisted snapshot. Cuts startup menu-nav load (and wedge risk) |
