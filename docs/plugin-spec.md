...
| rs485bridge backend | **DONE — production primary** | `RS485BridgeBackend` in `pool_service.py` drives the pad daemon over HTTP/Tailscale, mirroring `AquaConnectBackend` (`lcd` + `send_nav_key`) so `MenuNavigator` / `_ac_set_circuit` / prefetch work unchanged. `--backend rs485bridge --rs485bridge-host pool` (the pad's MagicDNS name, which survives a tailnet-IP change; a raw `<pad-tailnet-ip>` also works); token via `RS485_BRIDGE_TOKEN` (never persisted). §4.4. Plugin `config.schema.json` + `platform.ts` wiring, HEATER_1 path, and production cutover all shipped; it's the live primary backend. |
