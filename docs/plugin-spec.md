...
| Verify pool active-slot highlight | Backlog | Cockpit highlights the running pool speed from `vsp_active_slot`, parsed from the panel's scrolling `Filter On:Spd(\d)` line. Confirm the regex matches the real panel text on hardware; right after a restart the field is `None` until that line scrolls past, so no slot lights up momentarily. If it never lights while the filter runs, the panel text differs from the regex — capture the actual display string and adjust |
