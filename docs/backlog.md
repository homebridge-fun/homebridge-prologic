...
| ~~1.2~~ | ~~**Frame-corpus plumbing + first review pass**~~ | — | **Done.** `scripts/harvest_frames.py` (shape-based dedupe, `--append`, `--coverage`), `sidecar/tests/frames.jsonl`, and `test_frame_corpus.py` which replays it. **All 74 captured frames are now reviewed** — named, with the reason for each expectation recorded, and `reviewed: true`; a test requires the reason. The review found and fixed a real parser gap (the `_F` degree encoding silently dropping heater setpoints). What remains is *ongoing*, not open: new shapes arrive from the ledger (1.7), get harvested, and get reviewed the same way. |
...
| 1.12 | **Audit for more "flags that cannot go false"** | Small | Three found in two weeks, all the same shape: a health signal only ever written on the success path, so failure leaves the last good value in place. The pad daemon's `_connected` (set False only when `process()` raises — a stale fd never raises), the sidecar's `state.connected` (fixed), and `/health`'s `ok` which is a literal `True`. Worth sweeping for others: any flag whose assignment appears once, inside a `try` that succeeded. |
| 1.11 | **Other install-time values that could go stale the same way** | Investigate | The pad's bind address was frozen at install time and silently broke on a renumber (now fixed). `install-pad.sh` resolves other things once too — the serial device path (`/dev/ttyUSB0`, which can move if the FTDI adapter is re-enumerated) and the repo checkout path. Worth an audit for the same shape of bug: a value captured once, used forever, with no check that it is still true. |
