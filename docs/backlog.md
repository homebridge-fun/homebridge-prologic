...
| 1.7 | **Continuous frame capture in the sidecar** | Small | The harvester samples `lcd.snapshot()`, a `deque(maxlen=60)` — the last 60 frames, minutes at most — and only when run by hand. So the *rare* frames the corpus most needs (a real fault, freeze protection, the 5–10 s VSP window) are exactly the ones it will miss: catching one means running a harvest within minutes of it appearing. Fix: compute the shape in `LcdCapture.text_updated` (where `_note_super_chlor_frame` already hooks every frame), keep an in-process set of seen shapes, and persist novel ones. After warmup that's a set lookup per frame and essentially no writes. Then harvest reads that log instead of the ring, and a frame the panel showed at 3 a.m. is still there in the morning. **This is what makes Tier B's "accrues over months" true rather than aspirational.** |
