...
| 1.8 | **Read the panel data a menu walk revealed** | Small each | A real menu walk surfaced screens the sidecar sees but ignores. Highest value first: **cell diagnostics** `-25.31V -5.81A 76°F 2900 PPM` (salt-cell voltage/current — genuine health data, and a failing cell shows here before it shows anywhere else); **Flow Switch** state; **Instant Salt** `2900 PPM(+=save)`, which disagrees with the scroll's `Salt Level 3000 PPM` and is worth understanding; firmware revisions (Main 4.46, RF Base r3.00 ID:203A, Display, VSP) — useful in bug reports from other installs. |
| 1.9 | **Filter timers are readable** | Medium | `Filter T1-all 07:00A to 08:00A`, `T2` 8:00A–9:30P, `T3` 9:30P–10:30P, `T4 --- Off ---`, plus `Spa-all` and `Valve3-all`, and `Filter T1-Spd1 90%` linking a timer to a VSP speed. The README lists timers/schedules as panel-only and unsupported. They are plainly parseable — reading them (not writing) would let HomeKit and the cockpit show what the pool is actually scheduled to do. |
